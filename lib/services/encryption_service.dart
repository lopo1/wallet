  ...
  static String encrypt(String data, String password) {
    try {
      ...
      if (password.length < 8) {
        throw const EncryptionException('密码长度至少为8位');
      }
  ...
  static Uint8List _aesEncrypt(List<int> data, Uint8List key, Uint8List iv) {
    try {
      ...
      while (offset < paddedData.length) {
        final processed = cipher.processBlock(paddedData, offset, encrypted, offset);
        if (processed == 0) {
  ...
  static Uint8List _aesDecrypt(List<int> encryptedData, Uint8List key, Uint8List iv) {
    try {
      ...
      while (offset < encryptedData.length) {
        final processed = cipher.processBlock(Uint8List.fromList(encryptedData), offset, decrypted, offset);
        if (processed == 0) {
