...
class StorageService {
  ...
  Future<void> saveWallet(Wallet wallet, String password) async {
    try {
      ...
      final encryptedMnemonic = EncryptionService.encrypt(wallet.mnemonic, password);
  ...
  Future<String?> getMnemonic(String walletId, String password) async {
    try {
      ...
      // 使用AES解密助记词
      final decryptedMnemonic = EncryptionService.decrypt(encryptedMnemonic, password);
      ...
      // 清理助记词（去除多余空格和换行符）
      final cleanedMnemonic = decryptedMnemonic.trim().replaceAll(RegExp(r'\s+'), ' ');
  ...
  Future<void> updateWalletAddresses(String walletId, Map<String, List<String>> addresses) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final walletsJson = prefs.getStringList(_walletsKey) ?? [];
  ...
  Future<void> updateWalletAddressesAndIndexes(String walletId, Map<String, List<String>> addresses, Map<String, int> addressIndexes, [Map<String, String>? addressNames]) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final walletsJson = prefs.getStringList(_walletsKey) ?? [];
  ...
  Future<bool> verifyPassword(String walletId, String password) async {
    try {
      ...
      try {
        final prefs = await SharedPreferences.getInstance();
        final encryptedMnemonic = prefs.getString('$_encryptedMnemonicPrefix$walletId');
        if (encryptedMnemonic != null) {
          final decrypted = EncryptionService.decrypt(encryptedMnemonic, password);
